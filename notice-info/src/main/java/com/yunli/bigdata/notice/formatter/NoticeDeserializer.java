package com.yunli.bigdata.notice.formatter;
...
/**
 * @author pingchangxin
 * @description 消通知息反序列化
 * @date 2021/6/22
 **/
@Slf4j
public class NoticeDeserializer implements Deserializer {
    ...
    public Serializable deserialize(byte[] bytes) {
        try {
            ...
            if (messageData != null && messageData.getType() != null ) {
                ...
                log.error("通知消息返序列化为空");
            }
        } catch (IOException e) {
            log.error("通知消息反序列异常", e);
        }
        return null;
    }
